...
class VoiceCloner:
    ...
    def text_to_speech(self, text: str, output_path: str, voice_id: str = None):
        try:
            logger.info(f"🗣️ Generating TTS with MiniMax...")

            tts_url = f"https://api.minimaxi.chat/v1/t2a_v2?GroupId={self.group_id}"
            ...
            response = requests.post(tts_url, headers=self.headers, json=payload, timeout=60)
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ MiniMax TTS failed: {response.status_code} - {response.text}")
                return False
